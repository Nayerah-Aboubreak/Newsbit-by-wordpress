 ...
 */
class frannawp_Category_One_Widget extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			'category_one_widget', // Base ID
			esc_html__( 'Category 1 Posts', 'frannawp' ), // Name
	...
	public function widget( $args, $instance ) {
        ...
        if ( $type == 'category' ) {
            $args_post_one_left['category__in'] = $category;
        }

        $post_cat_one_left   =   new WP_Query( $args_post_one_left );
    ?>

    <!-- Widget Category One Post 1 Starts -->

    <div class="widget-category-one">

        <?php while( $post_cat_one_left->have_posts() ): $post_cat_one_left->the_post(); ?>

            <?php $post_one =   $post->ID; ?>
            
            <div class="widget-category-one-left">
        ...
        <?php
            ...
            // Display from chosen category.
            if ( $type == 'category' ) {
                $args_post_one_right['category__in'] = $category;
            }

            $post_cat_one_right   =   new WP_Query( $args_post_one_right );

        ?>

        <div class="widget-category-one-right">

            <?php while( $post_cat_one_right->have_posts() ): $post_cat_one_right->the_post(); ?>

                <div class="widget-category-one-right-content">
                    <div class="img-container">
                        <a href="<?php the_permalink(); ?>">
                            <?php if( has_post_thumbnail() ) :?>
                                <?php frannawp_post_thumbnail(); ?>
